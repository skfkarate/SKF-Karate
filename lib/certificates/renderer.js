export class CertificateRenderer {
  ...
  async drawQrCode(url, x, y, size) {
    // We would use qrcode library here to generate a DataURL and drawImage
    // For now, draw a mock square
    const xPx = (x / 100) * this.baseWidth
    const yPx = (y / 100) * this.baseHeight
    const sizePx = (size / 100) * this.baseWidth

    this.ctx.fillStyle = '#ffffff'
    this.ctx.fillRect(xPx - sizePx/2, yPx - sizePx/2, sizePx, sizePx)
    
    this.ctx.fillStyle = '#000000'
    this.ctx.fillRect(xPx - sizePx/2 + 10, yPx - sizePx/2 + 10, sizePx - 20, sizePx - 20)
    
    this.ctx.fillStyle = '#ffffff'
    this.ctx.font = `${sizePx/6}px sans-serif`
    this.ctx.textAlign = 'center'
    this.ctx.textBaseline = 'middle'
    this.ctx.fillText('QR CODE', xPx, yPx)
  }
