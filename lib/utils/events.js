...
export function getStudentEventEntries(student, events = []) {
  return events.flatMap((event) => {
    const participants = event.participants || event.winners || []

    return participants
      .filter((participant) => {
        if (participant.registrationNumber && student.registrationNumber) {
          return participant.registrationNumber === student.registrationNumber
        }

        return participant.studentName === `${student.firstName} ${student.lastName}`
      })
      .map((participant) => ({
        eventId: event.id,
...
export function buildResultCascadeSummary({ student, event, result }) {
  return {
    ...
    cacheKeys: [`student:${student.registrationNumber}`, `event:${event.slug}`, "rankings"],
  }
}
