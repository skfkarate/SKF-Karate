...
import { buildCompetitionResultsFromStudents, getStudentRankEntry } from '../utils/rankings';
...
const STUDENTS_DATA_FILE = resolveDataFile('students.json');
...
let mockStudents = [
  ...
  },
  {
    ...
    achievements: [
      ...
      { id: 'ach_6_4', type: 'referral-bonus', date: '2023-11-20', title: 'Referred a New Student', pointsAwarded: 150 }
    ],
    pointsHistory: []
  },
  {
...
let studentsLoadedFromDisk = false;

function cloneStudentData(value) {
  return JSON.parse(JSON.stringify(value));
}

function ensureStudentsLoaded() {
  if (studentsLoadedFromDisk) return;
  studentsLoadedFromDisk = true;

  try {
    const stored = readJsonArray(STUDENTS_DATA_FILE);
    if (Array.isArray(stored) && stored.length > 0) {
      mockStudents = stored;
    }
  } catch (error) {
    console.error('Failed to load student store:', error);
  }
}

function persistStudents() {
  ensureStudentsLoaded();
  writeJsonAtomically(STUDENTS_DATA_FILE, mockStudents);
}

export function getAllStudents() {
  ensureStudentsLoaded();
  return cloneStudentData(mockStudents);
}

export function getStudentByRegistrationNumber(regNum) {
  ensureStudentsLoaded();
  const student = mockStudents.find(s => s.registrationNumber.toUpperCase() === regNum.toUpperCase()) || null;
  return student ? cloneStudentData(student) : null;
}
...
export function getStudentsByBranch(branch) {
  ensureStudentsLoaded();
  return cloneStudentData(
    mockStudents.filter(s => s.branchName.toLowerCase() === branch.toLowerCase())
  );
}
...
export function getFeaturedStudents() {
  ensureStudentsLoaded();
  return cloneStudentData(
    mockStudents.filter(s => s.isFeatured && s.isPublic && s.status === 'active')
  );
}

export function getNextSequenceNumber(year) {
  ensureStudentsLoaded();
  const studentsThisYear = mockStudents.filter(s => s.registrationNumber.startsWith(`SKF-${year}`));
  if (studentsThisYear.length === 0) return 1;
  const seqs = studentsThisYear.map(s => parseInt(s.registrationNumber.split('-')[2], 10));
  return Math.max(...seqs) + 1;
}

export function hasStudentRegistrationNumber(registrationNumber, excludeId = null) {
  ensureStudentsLoaded();
  const normalized = normaliseRegistrationNumber(registrationNumber);

  return mockStudents.some((student) => {
    return (
      student.registrationNumber.toUpperCase() === normalized.toUpperCase() &&
...
export function getRankSnapshots() {
  ensureStudentsLoaded();
  const results = buildCompetitionResultsFromStudents(mockStudents);
  return calculateAllRanks(mockStudents, results);
}

export function getStudentRank(studentId) {
  ensureStudentsLoaded();
  const results = buildCompetitionResultsFromStudents(mockStudents);
  const rankInfo = getStudentRankEntry(studentId, mockStudents, results);
...
function normaliseStudentPayload(input = {}, existing = null) {
  ...
  return {
    id: existing?.id || input.id || `student_${randomUUID()}`,
    registrationNumber,
...
export function createStudent(input) {
  ensureStudentsLoaded();
  const student = normaliseStudentPayload(input);

  if (hasStudentRegistrationNumber(student.registrationNumber)) {
    throw new ApiError(409, 'A student with this registration number already exists.');
  }

  mockStudents = [student, ...mockStudents];
  persistStudents();
  return cloneStudentData(student);
}
...
export function updateStudent(id, input) {
  ...
  const updatedStudent = normaliseStudentPayload(input, mockStudents[index]);

  if (hasStudentRegistrationNumber(updatedStudent.registrationNumber, id)) {
    throw new ApiError(409, 'A student with this registration number already exists.');
  }

  mockStudents[index] = updatedStudent;
  persistStudents();
  return cloneStudentData(updatedStudent);
}
